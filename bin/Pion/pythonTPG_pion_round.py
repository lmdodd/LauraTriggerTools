...
for event in ntuple:
    #print ("genPt: %.2f" % event.genPt)
    #gen=round(event.genPt)
    #print ("genPt: %.2f" % gen)
    #ptbin=math.floor(gen/5)-2
    #print ("ptbin: %2f" % ptbin)
    for ptb in range(0,9):
        if event.ptbin==ptb:
        #if ptbin==ptb:
           for eta in range(0,56):
               SF=1.0
               if event.TPGh5x5>5 and event.TPGDiff==1:
                  ...
                  if event.TPG5x5>5 and event.TPGDiff==1 and event.TPG5x5_tpgeta==eta and event.TPGVeto>.25:
                     gen=event.genPt
		     #print ("gen: %.2f" % gen)
