          ...
          for ptb in range(0,9):
              if event.ptbin[i]==ptb:
                 for eta in range(0,56):
                     ...
                     if event.recoPt[i]>0 and event.TPG5x5[i]>3 and event.TPG5x5_tpgeta_[i] == eta and event.TPGVeto[i]>.95:
                        reco=event.recoPt[i]
...
#
file=ROOT.TFile("outfile_step2.root","RECREATE")
file.cd()
#
