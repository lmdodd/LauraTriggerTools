...
class LEffiTree : public edm::EDAnalyzer {
	public:
		explicit LEffiTree(const edm::ParameterSet& pset);
...
LEffiTree::LEffiTree(const edm::ParameterSet& pset){
...
unsigned int const LEffiTree::N_TOWER_PHI = 72;
unsigned int const LEffiTree::N_TOWER_ETA = 56;


void LEffiTree::analyze(const edm::Event& evt, const edm::EventSetup& es) {
	// Setup meta info
	run_ = evt.id().run();
